...
PROMPT="%{%(!.$fg_bold[red].$fg_bold[white])%}%n@%m%{$reset_color%} %{$fg_bold[blue]%}%~%{$reset_color%} %(!.☭.⚘) "
...
# commented out because it sends an error message for some reason???
# whatever the path is still correct
